  ...
  <div class="app-container">
    ...
    <main ref="inputCardRef" class="glass-card input-group">
      ...
      <div class="options-row" style="display: grid; grid-template-columns: repeat(auto-fit, minmax(200px, 1fr)); gap: 1rem;">
        ...
        <div class="selector-group">
          <label class="selector-label">解题视角</label>
          <div class="style-selector">
            <button 
      ...
      <div class="selector-group">
        <div style="display: flex; justify-content: space-between; align-items: center;">
          ...
          <div style="display: flex; gap: 0.5rem;">
            ...
            <button class="text-link-btn" @click="showGuideModal = true">物理模型图像与公式推导通关指南</button>
          </div>
        </div>
        <textarea 
          v-model="userInput" 
...
const appTitle = ref(appConfig.title || '网腾无限AI - 物理题目解析与公式原理专家');
...
const metricsList = [
  {
    "key": "physicalLogicRigor",
    "label": "物理逻辑严谨度"
  },
  {
    "key": "forceDiagramStepClarity",
    "label": "受力与过程拆解清晰度"
  },
  {
    "key": "formulaDerivationAccuracy",
    "label": "公式推导准确率"
  },
  {
    "key": "physicalIntuitionInsight",
    "label": "物理直觉与模型启发性"
  },
  {
    "key": "experimentalFeasibility",
    "label": "实验验证可操作分"
  }
];
const guideRules = [
  {
    "title": "隔离法与整体法结合",
    "advice": "受力分析时明确画出研究对象的受力图，区分内力与外力，选择合适的研究系统",
    "avoid": "切忌研究对象不明乱套牛顿第二定律"
  },
  {
    "title": "注意摩擦力突变临界",
    "advice": "在传送带或板块模型中，判断物体速度与传送带达到相等的瞬间，摩擦力方向是否发生突变",
    "avoid": "避免忽略临界点导致加速度计算错误"
  },
  {
    "title": "能量与动量双线验证",
    "advice": "涉及碰撞或弹簧系统，优先使用动量守恒与能量守恒方程，减少中间代数计算量",
    "avoid": "避免繁琐复杂的运动学公式死算"
  }
];
...
const displayResultText = computed(() => {
  ...
  return result.value.replace(/\[WULI_SCORES\][\s\S]*?\[\/WULI_SCORES\]/g, '').trim();
});

const parseAiScores = (rawText: string) => {
  const match = rawText.match(/\[WULI_SCORES\](.*?)\[\/WULI_SCORES\]/);
...
const loadHistory = () => {
  try {
    const raw = localStorage.getItem('wuli_history_records');
...
const saveHistory = () => {
  localStorage.setItem('wuli_history_records', JSON.stringify(historyList.value));
};
